...
class SaleOrder(models.Model):
	...
	def _calculate_helper_booleans(self):
		for record in self:
			if record.invoicing_property_id.inv_package_deal and record.invoicing_property_id.pay_in_terms:
				record.update({'inv_date_bool': False})
				record.update({'package': True})
				record.update({'terms_cond_bool': True})
			elif record .invoicing_property_id.pay_in_terms and not record.invoicing_property_id.inv_package_deal:
				record.update({'terms_cond_bool': True})
				record.update({'inv_date_bool': False})
				record.update({'package': False})
			elif record.invoicing_property_id.inv_package_deal and not record.invoicing_property_id.pay_in_terms:
				record.update({'inv_date_bool': True})
				record.update({'package': True})
				#self.inv_package_bool = True
				record.update({'terms_cond_bool': False})
			elif record.invoicing_property_id.inv_per_line_adv_print or record.invoicing_property_id.inv_per_line_adv_online or record.invoicing_property_id.inv_whole_order_at_once:
				record.update({'inv_date_bool': True})
			else:
				record.update({'inv_date_bool': False})
				record.update({'inv_date_bool': False})
