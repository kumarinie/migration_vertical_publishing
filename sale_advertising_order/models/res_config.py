...
class Partner(models.Model):
    ...
    def _compute_next_activities_count(self):
        for partner in self:
            ...
            partner.next_activities_count = self.env['crm.lead'].with_context({'lang':'en_US'}).search_count([('partner_id', operator, partner.id), ('type', '=', 'opportunity'), '|', ('is_activity', '=', True), ('activity_type_id', '!=', False), ('stage_id.name','!=','Logged')])

    @api.multi
    def _compute_activities_report_count(self):
        for partner in self:
            operator = 'child_of' if partner.is_company else '='  # the activity count should counts the activities of this company and all its contacts
            partner.activities_report_count = self.env['crm.lead'].with_context({'lang':'en_US'}).search_count([('partner_id', operator, partner.id), ('type', '=', 'opportunity'), '|', ('is_activity', '=', True), ('activity_type_id', '!=', False), ('stage_id.name','!=','Qualified'), ('stage_id.name','!=','Proposition')])
    ...
    def action_log(self):
        ...
        for log in self:
            body_html = "<div><b>%(title)s</b>: %(next_activity)s</div>%(description)s%(note)s" % {
                ...
                'next_activity': log.activity_type_id.name,
            ...
            log.lead_id.message_post(body_html, subject=summary, subtype_id=log.activity_type_id.subtype_id.id)
            log.lead_id.write({
